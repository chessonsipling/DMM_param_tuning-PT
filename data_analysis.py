...
#Extracts optimal parameters and performs analyses using DMM training data
def data_analysis(eqn_choice, prob_type, ns, flattened_big_ns):

    folder = f'training/{prob_type}/{flattened_big_ns}'
    param_dir = f'parameters/{prob_type}/{flattened_big_ns}'
    os.makedirs(folder, exist_ok=True)
    os.makedirs(param_dir, exist_ok=True)
    data = []
    if os.path.exists(f'{folder}/stats_{ns}.p'):
    ...
    #Extracts DMM statistics from training files
    data_size = len(data)
    ...
    for n_idx, ni in enumerate(ns):
        ...
        for i, stat_i in enumerate(relevant_stats):
            ...
            plt.savefig(f'{param_dir}/Stat_{stat_i}_{ni}.png', dpi=300, bbox_inches='tight')
            plt.close()
            ...
            plt.savefig(f'{param_dir}/Stat_{stat_i}_distribution_{ni}.png', dpi=300, bbox_inches='tight')
            plt.close()
    ...
    for i, param in enumerate(params):
        ...
        plt.savefig(f'{param_dir}/Param_{param}_vs_metric.png', dpi=300, bbox_inches='tight')
        plt.close()
        ...
        n_bins = int((best_param.max() - best_param.min()) / bin_size) ###some error here???
        n_bins = max(n_bins, 1)
        ...
        plt.savefig(f'{param_dir}/Param_{param}_distribution.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    for i, relevant_stat in enumerate(relevant_stats):
        ...
        ax.legend()
        plt.savefig(f'{param_dir}/Stat_{relevant_stat}_vs_n.png', dpi=300, bbox_inches='tight')
        plt.close()'''
