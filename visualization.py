...
folder = f'results/{prob_type}/Benchmark/scalability_comparison'
...
for i, file in enumerate(files):
    ...
    if i == 1:
        fit1 = np.polyfit(np.log(ns1[4:43]), np.log(step1[4:43]), 1)
        x_fit1 = np.linspace(np.min(ns1[4:43]), np.max(ns1[4:43]), 1000)
        ...
        x_fit1 = np.linspace(np.min(ns1[4:]), np.max(ns1[4:]), 1000)
    ...
    #Not able to fit to larger \zeta
        ...
        b_opt.append(b_opt1)
        c_opt.append(c_opt1)
    elif i == 23: #({\beta, \zeta} = {6.0\beta_{opt}, 1.5\zeta_{opt}})
        fit1 = np.polyfit(np.log(ns1[4:]), np.log(step1[4:]), 1)
        x_fit1 = np.linspace(np.min(ns1[4:]), np.max(ns1[4:]), 1000)
        y_fit1 = np.exp(fit1[1]) * x_fit1 ** fit1[0]
        print(f'alpha = {fit1[0]}')
    if i > 0:
        ns.append(ns1)
        step.append(step1)
    if i in [1, 9, 10, 12, 23]: #not able to fit for larger \zeta
        x_fit.append(x_fit1)
        y_fit.append(y_fit1)
...
params = [f'Optimal', r'$\beta = 10\beta_{opt}$',
          ...
          r'$\beta=6.0\beta_{opt}$'',\n'r'$\zeta=1.5\zeta_{opt}$']

#Plots median TTS as a function of N as a sccatterplot
ax.scatter(ns[0][4:43], step[0][4:43], label=f'{params[0]}', color='limegreen', marker='.') #original
...
ax.scatter(ns[8][4:], step[8][4:], label=f'{params[8]}', color='darkorange', marker='.') #10*\beta_{opt}, tuned dt_0
ax.scatter(ns[9][4:], step[9][4:], label=f'{params[9]}', color='darkorange', marker='*') #0.1*\beta_{opt}, tuned dt_0
#ax.scatter(ns[10][1:], step[10][1:], label=f'{params[10]}', color='blue', marker='.') #10*\zeta_{opt}, tuned dt_0
ax.scatter(ns[11][4:], step[11][4:], label=f'{params[11]}', color='blue', marker='*') #0.1*\zeta_{opt}, tuned dt_0
...
ax.scatter(ns[22][4:], step[22][4:], label=f'{params[22]}', color='black', marker='.') #(6.0, 1.5), tuned dt_0
...
ax.plot(x_fit[4], y_fit[4], color='black', linestyle='--')
...
plt.savefig(f'{folder}/combined_5_rescaled.png', dpi=300, bbox_inches='tight') #filename can be changed based on data which is plotted
plt.close()
